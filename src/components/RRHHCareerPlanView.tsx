...
const isTechnician = (activity: string) => /t[eé]cnico|hvt|alto voltaje/i.test(activity);
...
const formatDate = (date: Date | null) => date ? new Intl.DateTimeFormat('es-AR').format(date) : 'Sin fecha';
...
const RRHHCareerPlanView: React.FC<RRHHCareerPlanViewProps> = ({ items, searchQuery }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(item => {
      const matchesSearch = !normalizedSearch || [item.participante, item.actividad, item.provincia, item.genero, item.estado].some(value => value.toLowerCase().includes(normalizedSearch));
      return matchesSearch && (province === 'ALL' || item.provincia === province) && (employmentStatus === 'ALL' || (item.estado || 'Sin estado') === employmentStatus) && (executionStatus === 'ALL' || item.planificadoEjecutado === executionStatus);
    }).sort((a, b) => (calculatedDeadline(a)?.getTime() ?? Number.MAX_SAFE_INTEGER) - (calculatedDeadline(b)?.getTime() ?? Number.MAX_SAFE_INTEGER));
  }, [items, searchQuery, province, employmentStatus, executionStatus]);
  ...
  const summary = useMemo(() => {
    ...
    const overdue = filteredItems.filter(item => deadlineStatus(item).label === 'Vencido').length;
    const certified = filteredItems.filter(item => item.planificadoEjecutado.toLowerCase() === 'ejecutado').length;
  ...
  return <div className="space-y-8">
    ...
    <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard label="Certificaciones ejecutadas" value={summary.certified.toString()} detail="sobre el listado filtrado" icon={<CheckCircle2 size={22} />} tone="green" />
    ...
    <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-end gap-4"><div className="mb-2 flex items-center gap-2 pr-2 text-slate-400"><Filter size={17} className="text-[#00B0F0]" /><span className="font-display text-[10px] font-bold uppercase tracking-[0.2em]">Filtros</span></div><FilterSelect label="Provincia" value={province} onChange={setProvince} options={provinces} /><FilterSelect label="Estado" value={employmentStatus} onChange={setEmploymentStatus} options={employmentStatuses} /><FilterSelect label="Planificación" value={executionStatus} onChange={setExecutionStatus} options={executionStatuses} /></div></section>
