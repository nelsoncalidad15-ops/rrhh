...
import { CourseGrade, RelatorioItem, CollaboratorContact, CoursePhase, EstandarOperacionalItem, CareerPlanItem } from '../types';
        ...
        complete: (results) => {
          ...
          const items = rows
            .map(row => ({
              ...
              fin: String(getRowValue(row, 'Fin proyectado', 'Fin') ?? '').trim(),
              ...
              muestra: String(getRowValue(row, 'Muestra') ?? '').trim()
            }))
            .filter(item => (item.actividad || item.participante) && item.muestra.toLowerCase() !== 'no');
          resolve(items);
        },
        error: error => reject(error)
      });
